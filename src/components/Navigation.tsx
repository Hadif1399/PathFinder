      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'home' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Home</button>
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'quiz' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Career Quiz</button>
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'catalogue' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Career Catalogue</button>
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'universities' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Universities</button>
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'preuni-scholarships' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Pre-U Scholarships</button>
            <button onClick={() => dispatch({ type: 'NAVIGATE', page: 'scholarships' })} className="text-sm text-white/70 hover:text-neon-blue transition-colors">Scholarships</button>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
      ...
      {mobileOpen && (
        ...
        >
          <div className="px-4 py-4 space-y-3">
            ...
            <button onClick={() => { dispatch({ type: 'NAVIGATE', page: 'preuni-scholarships' }); setMobileOpen(false); }} className="block w-full text-left text-white/70 hover:text-neon-blue">Pre-U Scholarships</button>
