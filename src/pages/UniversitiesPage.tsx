...
import { commonPreUniversity, universitiesMalaysia, universitiesOverseas, University } from '../data/education';
import { useState, useMemo } from 'react';
import { GraduationCap, MapPin, Globe, DollarSign, Search, Building2, ArrowRight } from 'lucide-react';
            ...
            <div className="mb-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredMalaysia.filter((u) => !u.notes?.includes('Premier') && !u.notes?.includes('Top Private')).map((uni, i) => (
                  <UniversityCard key={i} uni={uni} color="blue" delay={i * 0.05} />
                ))}
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredMalaysia.filter((u) => u.notes?.includes('Top Private') || u.notes?.includes('Premier') || u.notes?.includes('industry') || u.notes?.includes('Petronas') || u.notes?.includes('Cyberjaya') || u.notes?.includes('Lancaster') || u.notes?.includes('affordable')).map((uni, i) => (
                  <UniversityCard key={i} uni={uni} color="purple" delay={i * 0.05} />
                ))}
                {filteredMalaysia.filter((u) => u.notes?.includes('medical') || u.notes?.includes('Medical') || u.notes?.includes('healthcare')).map((uni, i) => (
                  <UniversityCard key={`spec-${i}`} uni={uni} color="pink" delay={i * 0.05} />
                ))}
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredOverseas.map((uni, i) => (
                <UniversityCard key={i} uni={uni} color="purple" delay={i * 0.03} showCountry />
              ))}
            </div>
          ...
          <div className="glass-strong rounded-2xl p-8">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <motion.button
                ...
                className="px-6 py-3 rounded-full bg-gradient-to-r from-neon-yellow to-neon-orange text-white font-semibold"
              >
                💰 Pre-U Scholarships
              </motion.button>
              <motion.button
...
function UniversityCard({ uni, color, delay, showCountry }: { uni: University; color: string; delay: number; showCountry?: boolean }) {
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      whileHover={{ y: -3 }}
      className={`p-4 rounded-xl bg-white/5 border ${colorClasses[color]} transition-all`}
    >
      <div className="flex items-start justify-between mb-2">
        <div>
          {showCountry && uni.country && (
        ...
        {uni.ranking && (
          <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ml-2 ${
            ...
            color === 'purple' ? 'bg-neon-purple/10 text-neon-purple' :
      ...
      <div className="space-y-1.5 text-xs">
        ...
        {uni.notes && (
          <p className="text-white/40 italic mt-1 text-xs">{uni.notes}</p>
        )}
      </div>
    </motion.div>
  );
